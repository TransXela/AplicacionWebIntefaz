...
angular.module('transxelaWebApp')
  .controller('AdminPerfilCtrl', function ($scope, apiService, $cookies, $location) {
    if(typeof $cookies.getObject('user') !== 'undefined' && $cookies.getObject('user')){
      $scope.idadmin = $cookies.getObject('user').id;
      $scope.token = $cookies.getObject('user').token;
      $scope.admin = $cookies.getObject('user').usuario;
    }
    else{
      $location.url('/login');
    }
    ...
    $scope.cambiarContrasenia = function(){
      if($scope.cNueva === $scope.cRepetir) {
        apiService.modificar('/users/cambiarcontreniausuario/'+ $cookies.getObject('user').id + '/?tk=' + $scope.token, {
